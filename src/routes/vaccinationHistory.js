...
      const error = new Error('Las observaciones contienen contenido no permitido. Para reportar un incidente, por favor, use el endpoint /api/incidentes.');
      error.statusCode = 400;
      throw error;
    }
...
    let statusCode = 500;
    
    // Check for specific error messages from the stored procedure (RAISERROR)
    if (err.message.includes('La dosis aplicada debe ser un número positivo')) {
      errorMessage = err.message;
      statusCode = 400;
    } else if (err.message.includes('El ID del niño no existe en la base de datos')) {
      errorMessage = err.message;
      statusCode = 400;
    } else if (err.message.includes('El ID del usuario no existe en la base de datos')) {
      errorMessage = err.message;
      statusCode = 400;
    } else if (err.message.includes('El ID del centro no existe en la base de datos')) {
      errorMessage = err.message;
      statusCode = 400;
    } else if (err.message.includes('El ID del lote no existe en la base de datos o el lote es inválido')) {
      errorMessage = err.message;
      statusCode = 400;
    } else if (err.message.includes('El lote de vacuna no tiene stock suficiente')) {
      errorMessage = err.message;
      statusCode = 400;
    } else if (err.statusCode) { // Handle errors thrown by previous middlewares (e.g., validation)
      errorMessage = err.message;
...
    
    // Check for specific error messages from the stored procedure (RAISERROR)
    if (err.message.includes('El ID del historial no existe en la base de datos')) {
      errorMessage = err.message;
      statusCode = 400;
    } else if (err.statusCode) { // Handle errors thrown by previous middlewares (e.g., validation)
      errorMessage = err.message;
